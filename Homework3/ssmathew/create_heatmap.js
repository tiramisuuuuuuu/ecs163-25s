...
const  heatmapMargin = { top: 100, right: 0, bottom: 10, left: 300 };

// Create a responsive view box for the heatmap
const heatmapSvg = d3.select("#heatmap")
  .append("svg")
  .attr("preserveAspectRatio", "xMidYMid meet")
  .attr("viewBox", `-200 0 1700 2000`) // or whatever default size fits your data
  ...
  */
  ...
  for (i in myGroups) {
    for (j in myVars) {
      ...
      aggregated_data.push({ x: rowLabel, y: colLabel, value: sum/count, count: count })
    }
  }
  ...
  // Function to render heatmap info box based on selected heatmap square
  function renderBoxWithData(d) {
    console.log("Selected data:", d);
    let text1 = "Total num of students with " + d.x + " and " + d.y + ": " + d.count;
    let text2 = "Percentage of total who have above avg alchol use: " + (d.value*100).toFixed(2);
    document.getElementById("heatmaptext1").textContent = text1;
    document.getElementById("heatmaptext2").textContent = text2;
  }


  // Render the heatmap squares and onClick highlight the square using border and render the appropriate heatmap info box
  heatmapSvg.selectAll()
    ...
    .append("rect")
      ...
      .on("click", function(event, d) {
        ...
        if (selectedRect) {
          selectedRect.style("stroke", "none");
        }
        ...
        renderBoxWithData(aggregated_data[d]);
      });
  ...
  heatmapSvg.append("g")
    .attr("transform", `translate(500, ${100 + 10 + legendHeight})`)
    .call(legendAxis);
